...
(function (nMemcached) {
  ...
  memcached.connect = function connect(server, callback) {
    ...
    var serverTokens =
        server[0] === "/" ? server : /(.*):(\d+){1,}$/.exec(server).reverse(),
      memcached = this;
  ...
  memcached.command = function memcachedCommand(queryCompiler, server) {
    ...
    if (!server) {
      ...
      if (this.servers.length === 1) {
        ...
        if (redundancy && queryRedundancy) {
          redundancy = this.HashRing.range(
            query.key,
            this.redundancy + 1,
            true
          );
    ...
    if (!server || (server in this.issues && this.issues[server].failed)) {
      return (
        query.callback &&
        memcached.makeCallback(
          query.callback,
          new Error(["Server at", server, "not available"].join(" "))
        )
      );
    }
    ...
    this.connect(server, function allocateMemcachedConnection(error, S) {
      ...
      if (!S) {
        ...
        return (
          query.callback &&
          memcached.makeCallback(query.callback, new Error(message))
        );
      }
  ...
  privates.parsers = {
    ...
    SERVER_ERROR: function servererror(
      tokens,
      dataSet,
      err,
      queue,
      ...
      (memcached || this.memcached).connectionIssue(
        tokens.splice(1).join(" "),
        this
      );
      return [CONTINUE, false];
    },
    ...
    VALUE: function value(tokens, dataSet, err, queue) {
      var key = tokens[1],
        ...
        cas = tokens[4],
        multi =
          (this.metaData[0] && this.metaData[0].multi) || cas ? {} : false,
        tmp;
  ...
  privates.allCommands = new RegExp(
    "^(?:" + Object.keys(privates.parsers).join("|") + "|\\d" + ")"
  );
  privates.bufferedCommands = new RegExp(
    "^(?:" + Object.keys(privates.parsers).join("|") + ")"
  );
  ...
  memcached.rawDataReceived = function rawDataReceived(S) {
    ...
    while (
      S.bufferArray.length &&
      privates.allCommands.test(S.bufferArray[0])
    ) {
      ...
      if (privates.parsers[tokenSet[0]]) {
        ...
        resultSet = privates.parsers[tokenSet[0]].call(
          S,
          tokenSet,
          dataSet || token,
          err,
            ...
            if (metaData && metaData.callback) {
              ...
              this.delegateCallback(
                ...
                privates.resultParsers[metaData.type]
                  ? privates.resultParsers[metaData.type].call(
                      S,
                      resultSet,
                      err
                    )
        ...
        if (metaData && metaData.callback) {
          ...
          this.delegateCallback(
            metaData,
            new Error(
              'Unknown response from the memcached server: "' + token + '"'
            ),
            false,
            metaData.callback
          );
  ...
  memcached.getMulti = function getMulti(keys, callback) {
    ...
    function handle(err, results) {
      ...
      (Array.isArray(results) ? results : [results]).forEach(function each(
        value
      ) {
  ...
  privates.setters = function setters(
    type,
    validate,
    key,
    value,
    ...
    if (length > this.maxValue) {
      return privates.errorResponse(
        new Error("The length of the value is greater than " + this.maxValue),
        callback
      );
    }
  ...
  privates.addincr = function addincr(key, expire, callback) {
    ...
    this.command(function incredecrCommand(noreply) {
      return {
        ...
        type: "incr",
        redundancyEnabled: true,
        command: ["ma", fullkey, `N${expire}`, "J2", "v"].join(" "),
      };
    });
  };

  // Curry the function and so we can tell the type our private incrdecr
  memcached.increment = memcached.incr = curry(
    undefined,
    privates.incrdecr,
    "incr"
  );
  memcached.decrement = memcached.decr = curry(
    undefined,
    privates.incrdecr,
    "decr"
  );
